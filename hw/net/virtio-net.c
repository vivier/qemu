...
static void virtio_net_get_config(VirtIODevice *vdev, uint8_t *config)
{
    ...
    if (n->failover && !n->failover_default &&
        ...
        /*
         * if failover is enabled but not supported by the
         * guest, we can select to keep the primary device
         * by setting MAC address of virtio-net to 0.0.0.0,
         * this will force kernel to generate a different valid
...
static void virtio_net_set_features(VirtIODevice *vdev, uint64_t features)
{
    ...
    if (n->failover) {
        if (virtio_has_feature(features, VIRTIO_NET_F_STANDBY) ||
             ...
             *
             * The virtio-net device is disabled later by:
             * - post to virtio 1.0, by using the validate_feature() helper
             * - prior to 1.0, by setting its MAC address to 0.0.0.0
             */
