...
static void migration_completion(MigrationState *s)
{
    ...
    if (s->state == MIGRATION_STATUS_ACTIVE) {
        ...
        if (!s->parameters.pause_vm) {
            /* the vm state has been saved by pause_vm parameter */
            qemu_system_wakeup_request(QEMU_WAKEUP_REASON_OTHER, NULL);
                                    ...
                                    int new_state)
{
    if (qemu_savevm_state_guest_unplug_pending()) {
        ...
        /*
         * XXX: this is needed to finish unplug before to stop the VM
         * in pause_vm_after_unplug()
         */
        qemu_sem_timedwait(&s->wait_unplug_sem, 250);
...
static void pause_vm_after_unplug(MigrationState *s)
{
    if (s->parameters.pause_vm) {
        ...
        qemu_system_wakeup_request(QEMU_WAKEUP_REASON_OTHER, NULL);
        s->vm_was_running = runstate_is_running();
        if (global_state_store() || vm_stop_force_state(RUN_STATE_PAUSED)) {
