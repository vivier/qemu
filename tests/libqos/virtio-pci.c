...
static void qvirtio_pci_msix_set_message(QVirtioPCIDevice *d, int entry,
                                         struct MSIMessage msg)
{
    void *addr = d->pdev->msix_table + (entry * 16);
    uint32_t control;
    ...
    qpci_io_writel(d->pdev, addr + PCI_MSIX_ENTRY_UPPER_ADDR,
                                            (msg.address >> 32) & ~0UL);
    qpci_io_writel(d->pdev, addr + PCI_MSIX_ENTRY_DATA, msg.data);
