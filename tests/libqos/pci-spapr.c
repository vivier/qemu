...
#define QPCIREAD(a, v) do { \
    uint64_t port = (uint64_t)addr; \
    if (port < SPAPR_PCI_IO_WIN_SIZE) { \
        memread(IOBASE(0) + port, &(v), sizeof(v)); \
    } else { \
        memread(MMIOBASE(0) + port, &(v), sizeof(v)); \
    } \
} while (0)
...
#define QPCIWRITE(a, v) do { \
    uint64_t port = (uint64_t)addr; \
    if (port < SPAPR_PCI_IO_WIN_SIZE) { \
        memwrite(IOBASE(0) + port, &(v), sizeof(v)); \
    } else { \
        memwrite(MMIOBASE(0) + port, &(v), sizeof(v)); \
    } \
} while (0)

static void qpci_spapr_io_writeb(QPCIBus *bus, void *addr, uint8_t value)
{
    QPCIWRITE(addr, value);
}
