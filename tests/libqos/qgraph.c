...
#define CHECK_NODE_OPTIONS(opts)                            \
    do {                                                    \
        if (!opts) {                                        \
            opts = &(QOSGraphNodeOptions) { };              \
            memset(opts, 0, sizeof(QOSGraphNodeOptions));   \
        }                                                   \
    } while(0)
...
struct QOSGraphEdge {
    QOSEdgeType type;
    char *dest;
    void *arg; /* just for QEDGE_CONTAINS and QEDGE_CONSUMED_BY */
    char *command_line;
 ...
 * Adds also an optional command line arg of size @size.
 */
                    ...
                    QOSEdgeType type, QOSGraphEdgeOptions *opts)
{
    ...
    if (!opts) {
        ...
        memset(opts, 0, sizeof(QOSGraphEdgeOptions));
    }
    ...
    if (opts->cmd_line) {
        edge->command_line = g_strconcat(",", opts->cmd_line, NULL);
    }
    if (opts->size_arg) {
...
char *qos_graph_get_edge_cmd_line(QOSGraphEdge *edge)
{
    if (!edge) {
        return NULL;
    }
    return edge->command_line;
}
...
{
    qos_add_test_args(name, driver, test_func, NULL);
}
                        ...
                        QOSTestFunc test_func, QOSGraphTestOptions *opts)
{
    if (!opts) {
        ...
        memset(opts, 0, sizeof(QOSGraphTestOptions));
    }
    QOSGraphNode *node = create_node(name, QNODE_TEST);
    build_test_cmd_line(node, opts->opts.extra_args);
    node->u.test.function = test_func;
    node->u.test.arg = g_memdup(opts->test_arg, opts->size_test_arg);
    node->u.test.before = opts->before;
    node->u.test.after = opts->after;
    node->available = TRUE;
    add_edge(driver, name, QEDGE_CONSUMED_BY, NULL);
}
...
{
    CHECK_NODE_OPTIONS(opts);
    QOSGraphNode *node = create_node(name, QNODE_MACHINE);
    build_machine_cmd_line(node, opts->extra_args);
...
{
    add_edge(producer, produced, QEDGE_PRODUCES, NULL);
}

void qos_node_consumes(const char *consumer, const char *consumed)
{
    add_edge(consumed, consumer, QEDGE_CONSUMED_BY, NULL);
}

void qos_node_consumes_args(const char *consumer, const char *consumed,
                            QOSGraphEdgeOptions *opts)
{
    add_edge(consumed, consumer, QEDGE_CONSUMED_BY, opts);
}
...
void qos_graph_node_set_availability(const char *node, bool av)
{
    ...
    QSLIST_FOREACH_SAFE(e, elist, edge_list, next) {
        if (e->type == QEDGE_CONTAINS || e->type == QEDGE_PRODUCES) {
            qos_graph_node_set_availability(e->dest, FALSE);
        }
    }
}
