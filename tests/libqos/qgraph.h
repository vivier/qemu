...
struct QOSGraphTestOptions {
    ...
    QOSGraphNodeOptions opts; /* extra args for test (add device) */
 ...
 * by @node with availability TRUE.
 *
 * @av is used to set whether to keep the node command line or not:
 * this is done while querying QEMU via QMP for a list of available
 * devices, and the node matching will keep its command line,
 * while all its node contained or consumed won't, since they
 * do not match QEMU list of devices.
 */
