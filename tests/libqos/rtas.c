...
static unsigned int qrtas_busy_delay_time(int status)
{
    int order;
    unsigned int ms = 0;
...
static unsigned int qrtas_busy_delay(int status)
{
    ...
    ms = qrtas_busy_delay_time(status);
    if (ms) {
        usleep(ms * 1000);
    }
                     ...
                     uint32_t addr, uint32_t func, uint32_t num_irqs)
{
    ...
    do {
        res = qrtas_call(qts, alloc, "ibm,change-msi", 6, args, nret, ret);
        if (res < 0) {
            return -1;
        }
        args[5] = ret[2];
    } while (qrtas_busy_delay(ret[0]));

    if (ret[0] != 0) {
        return -1;
    }

    /* on success, return the number of irqs allocated */
    return ret[1];
}
                           ...
                           uint64_t buid, uint32_t addr, int offset)
{
    ...
    do {
        res = qrtas_call(qts, alloc, "ibm,query-interrupt-source-number", 4,
                         args, 3, ret);
        if (res < 0) {
            return -1;
        }
