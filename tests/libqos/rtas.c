...
int qrtas_ibm_change_msi(QGuestAllocator *alloc, uint64_t buid,
                         ...
                         uint32_t *intr_type)
{
    ...
    do {
        res = qrtas_call(alloc, "ibm,change-msi", 6, args,
                         intr_type ? 4 : 3, ret);
        if (res != 0) {
            return -1;
        }
...
uint32_t qrtas_ibm_query_interrupt_source_number(QGuestAllocator *alloc,
                                                 ...
                                                 uint32_t ioa_intr_num)
{
    ...
    res = qrtas_call(alloc, "ibm,query-interrupt-source-number", 4, args,
                     3, ret);
    if (res != 0) {
        return -1;
    }
