...
{
    QOSGraphObject *garbage_collector[QOS_PATH_MAX_ELEMENT_SIZE];
    int garbage_size = 0;
    QOSEdgeType etype;
    QOSGraphNode *node;
    QOSGraphObject *o;
...
static void walk_path(QOSGraphNode *orig_path, int len)
{
    ...
    char *ro_cmd_line[(QOS_PATH_MAX_ELEMENT_SIZE + 1)];
    char *cmd_line_gc[(QOS_PATH_MAX_ELEMENT_SIZE + 1)];
    int ro_cmd_line_size = 0, ro_path_size = 0, cmd_line_gc_size = 0;
    int add_cmd_line = 0;
    char *machine = NULL, *arch = NULL, *str_path, *cmd_line;
    char *tmp = orig_path->name;

    do {
        path = qos_graph_get_node(tmp);
        tmp = qos_graph_get_edge_dest(path->path_edge);

        if (path->command_line && add_cmd_line) {
            ro_cmd_line[ro_cmd_line_size] =
                g_strconcat(path->command_line, cmd_line, NULL);
            cmd_line_gc[cmd_line_gc_size++] = ro_cmd_line[ro_cmd_line_size];
            ro_cmd_line_size++;
            add_cmd_line = 0;
        }
        ...
        if (path->type == MACHINE) {
            ...
            ro_cmd_line[ro_cmd_line_size++] = path->command_line;
        } else {
            ro_path[ro_path_size++] = path->name;
            etype = qos_graph_get_edge_type(path->name, tmp);
            add_cmd_line = etype == CONSUMED_BY;
            if (add_cmd_line) {
                cmd_line = qos_graph_get_edge_arg(path->path_edge);
            }
        }
    ...
    ro_cmd_line[ro_cmd_line_size] = NULL;
    cmd_line = g_strjoinv(" ", ro_cmd_line);
    ...
    for (int i = 0; i < cmd_line_gc_size; i++) {
        g_free(cmd_line_gc[i]);
    }
}
