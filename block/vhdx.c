...
fail:
    qerror_report(QERR_GENERIC_ERROR, "No valid VHDX header found");
    qemu_vfree(header1);
    qemu_vfree(header2);
    s->headers[0] = NULL;
    s->headers[1] = NULL;
...
static int vhdx_create(const char *filename, QEMUOptionParameter *options)
{
    ...
    if (image_size > VHDX_MAX_IMAGE_SIZE) {
        error_report("Image size too large; max of 64TB");
        ret = -EINVAL;
        goto exit;
    }
    ...
    if (!strcmp(type, "dynamic")) {
        ...
        error_report("Differencing files not yet supported");
        ret = -ENOTSUP;
        goto exit;
    } else {
        ret = -EINVAL;
